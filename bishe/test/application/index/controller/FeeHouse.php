...
//        FeeHouseModel::create($data);
    ...
    public function search(Request $request){
        ...
        $list=FeeHouseModel::where($condition)->paginate(20,false,[
    ...
    public function isdo(){
        ...
        $list=FeeHouseModel::where('status','=','1')->paginate(20,false,[
            'type'=>'BootstrapDetailed'
        ]);
    ...
    public function undo(){
        ...
        $list=FeeHouseModel::where('status','=','0')->paginate(20,false,[
            'type'=>'BootstrapDetailed'
        ]);
        $this->view->count=count($list);
...
//        FeeHouseModel::update(['delete_time'=>NULL],['is_delete'=>1]);//无用
    ...
    public function feehouseadd2(){
        $this->isLogin();//判断用户是否登录

        $this->view->assign('title','批量添加物业费');
        return $this->fetch('fee_house_add2');
    }
    ...
    public function addFeehouse2(Request $request){
        $this->isLogin();//判断用户是否登录
        $name=$request->param('name');
        $money=$request->param('money');
        $start_time=$request->param('start_time');
        $building_type=$request->param('building_type');
        $building_id=BuildingModel::where('type','=',$building_type)->column('id');
        $parking=HouseModel::where('building_id','in',$building_id)->where('status','=',1)->select();
        $data=[
            'type'=>'2',
            'name'=>$name,
//            'money'=>$money,
        ...
        foreach ($parking as $value){
            $list=$value->toArray();
            $data['house_id']=$list['id'];
            $data['money']=$list['area']*$money;
            FeeHouseModel::create($data);
        }
        $status=1;
        $message='添加成功';
